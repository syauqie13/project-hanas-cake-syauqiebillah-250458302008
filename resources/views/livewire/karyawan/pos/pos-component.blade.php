                            ...
                            @forelse ($products as $product)
                                <div wire:key="product-{{ $product->id }}">
                                    ...
                                    <div class="product-card {{ $product->stock <= 0 ? 'disabled' : '' }}"
                                        ...
                                        <img src="{{ $product->image ? asset('storage/' . $product->image) : 'https://placehold.co/150x150/e2e8f0/cbd5e0?text=Produk'}}"
                                        ...
                                        <div class="product-info">
                                            ...
                                            <div class="product-stock">
                                                <span
                                                    class="badge-stock{{ $product->stock <= 0 ? 'empty' : ($product->stock < 5 ? 'low' : '') }}"><i
...
<div
    wire:key="payment-tunai-section"
    x-data="{
        displayValue: '{{ $paid_amount == '' ? '' : number_format($paid_amount, 0, ',', '.') }}',
        ...
        <div class="input-group">
            ...
            <input
                ...
                @input.debounce.300ms="
                    let clean = cleanRupiah($event.target.value);
                    $wire.set('paid_amount', clean);
                    displayValue = formatRupiah(clean);
                "

                @blur="displayValue = formatRupiah(displayValue)"
            >
        </div>
    </div>
    ...
    <script>
        document.addEventListener('livewire:init', () => {
            ...
            Livewire.on('print-receipt', (data) => {
                console.log('Event print-receipt DITERIMA!', data);
                ...
                iframe.onload = function () {
                    console.log('Fungsi ONLOAD terpicu...'); // <-- Log tambahan
                    ...
                    if (!hasPrinted && iframe.src.includes(printUrl)) {
                        ...
                        setTimeout(() => {
                            ...
                            // Beri tahu Livewire untuk membersihkan keranjang SEKARANG
                            console.log('Memberi tahu Livewire untuk clear cart...');
                            Livewire.dispatch('clear-pos-cart');
